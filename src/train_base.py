...
def train_gap(model,
    ...
    for epoch in range(EPOCH_NUM):
        ...
        for batch in tqdm(train_loader):
            ...
            sim_loss = gen_sim_loss(pred, samples.contiguous().view(-1), sim_rewards)
            dis_loss = gen_dis_loss(pred, samples.contiguous().view(-1), dis_rewards)
            adv_loss = gen_adv_loss(pred, samples.contiguous().view(-1), adv_rewards)
